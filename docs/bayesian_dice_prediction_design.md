...
## 11. 理解確認問題と答え
### 問題
1. なぜ Dirichlet 分布を事前分布として選ぶと、更新が実装しやすくなりますか。
2. 観測回数 $n_k$ が増えると、事後分布のパラメータはどのように変化しますか。
3. このプロジェクトで事後平均と事後予測分布が同じ形になるのは、どのモデルを採用しているからですか。
4. ブラックボックス制約は、評価や学習の公平性にどう役立ちますか。
5. 初期実装で Recall や F1 を主指標にしていないのはなぜですか。

### 答え
1. Dirichlet 分布はカテゴリ分布に対する共役事前分布なので、観測後の事後分布を閉形式で計算できます。そのため、各面のカウントを足すだけで更新できます。
2. 各面について $\alpha_k + n_k$ の形に増加します。つまり、観測された回数がそのまま事後分布の強さに反映されます。
3. Dirichlet-Categorical モデルを採用しているからです。この共役関係により、次の出目の予測確率は事後平均と同じ式で表せます。
4. 予測器が真の内部確率を盗み見ずに、観測データだけで学ぶことを保証できます。これにより、実運用に近い条件で性能を検証できます。
5. この課題の主目的は、確率分布の推定とその不確実性の把握だからです。単一ラベル分類の成績だけでは、確率予測の質を十分に評価できません。
